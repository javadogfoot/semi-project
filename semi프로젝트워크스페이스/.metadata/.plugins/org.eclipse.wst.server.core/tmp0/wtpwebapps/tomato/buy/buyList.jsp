...
<script>
function moveSellingPage(){
   location.href='/tomato/buy/buyList_selling.jsp?col=s_idx&range=desc';
}
function moveSearch(){
...
<section>
   <article>
      ...
      <input type="button" value="검색" id="joinbtn1" onclick="moveSearch();">
      </div>
      </form>
   </article>
   <article id="sort">
   ...
   %>
      <a href="/tomato/buy/buyList.jsp?col=s_idx&range=desc" class="<%=newSort%>">최신순</a>|
      <a href="/tomato/buy/buyList.jsp?col=s_count&range=desc" class="<%=populSort%>">인기순</a>|
      <a href="/tomato/buy/buyList.jsp?col=s_price&range=desc" class="<%=hPriceSort%>">높은가격순</a>|
      <a href="/tomato/buy/buyList.jsp?col=s_price&range=asc" class="<%=lPriceSort%>">낮은가격순</a>|
   </article>
<hr>
</section>
<section>
      ...
      <tbody>
         ...
         <tfoot>
            <tr>
               ...
               if(userGroup!=0){
                  %>
                  <a href="buyList.jsp?cp=1&col=<%=col%>&range=<%=range%>">%lt;&lt;</a>
                  <a href="buyList.jsp?cp=<%=(userGroup-1)*pageSize+pageSize %>&col=<%=col%>&range=<%=range%>">&lt;</a>
                  <%
               }
               %>
               <%
               for(int i=userGroup*pageSize+1;i<userGroup*pageSize+pageSize;i++){
                  ...
                  &nbsp;&nbsp;<a href="buyList.jsp?cp=<%=i %>&col=<%=col%>&range=<%=range%>"><%=i %>
                  </a>&nbsp;&nbsp;</label>
                  <%
                  if(i==totalPage){
                     break;
               ...
               if(userGroup!=(totalPage/pageSize)-(totalPage%pageSize==0?1:0)){
                  %>
                  <a href="buyList.jsp?cp=<%=(userGroup+1)*pageSize+1 %>&col=<%=col%>&range=<%=range%>">&gt;</a>
                  <a href="buyList.jsp?cp=<%=totalPage %>&col=<%=col%>&range=<%=range%>">&gt;&gt;</a>
                  <%
               }
               %>
               </td>
...
<div style="float: right;">
   <img src="/tomato/img/noncheck.png" style="height:20px;width:20px;border:2px solid teal;" onclick="moveSellingPage();">
   &nbsp;
